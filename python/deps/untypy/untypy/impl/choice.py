...
class Choice:
    ...
    def __class_getitem__(cls, args):
        if len(args) < 2:
            raise TypeError(f'Choice requires at least two arguments: Choice[TYPE_1, ..., FUN]')
...
class ChoiceChecker(TypeChecker):
    ...
    def __illegal_use(self):
        raise TypeError(f"{self.describe()} can only be used as the return type of a function or method")
