...
class TotalSizeVisitor(FileSystemVisitor):
    ...
    def visitDirectory(self, dir: Directory):
        for c in dir.getChildren():
            c.accept(self)
    def visitFile(self, f: File):
        # Name of parameter should be irrelevant. The parameter is named
        # file in the superclass, but f here. This should not matter (but it did).
        self.__totalSize += len(f.getContent())
