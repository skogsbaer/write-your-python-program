...
export async function writeBackendTrace(backendTrace: BackendTrace, path: string) {
    await vscode.workspace.fs.writeFile(vscode.Uri.file(path), new util.TextEncoder().encode(stringify(backendTrace)));
}
